...
const MANAGEMENT_MENU = [
  ...
  { to: '/inventory', icon: <Shirt size={18} />, label: 'Loja', roles: ['superuser', 'admin', 'instructor', 'staff', 'student'] },
...
const App: React.FC = () => {
  ...
  const t_local = getTranslation(language);
  ...
  const translatedManagementMenu = [
    ...
    { to: '/inventory', icon: <Shirt size={18} />, label: t.inventory, roles: ['superuser', 'admin', 'instructor', 'staff', 'student'] },
    { to: '/instructor-profile', icon: <UserCircle size={18} />, label: 'Meus Dados', roles: ['instructor', 'staff'] },
  ...
  useEffect(() => {
    // Storage handles language persistence through the context now if we want, 
    // but we can keep this for safety or remove it since LanguageProvider already handles it.
  }, [language]);
          ...
          <div className="md:hidden fixed inset-0 z-[100] flex items-end">
            ...
            <div className="relative w-full bg-white dark:bg-slate-900 rounded-t-[40px] shadow-2xl animate-in slide-in-from-bottom duration-500 p-8 flex flex-col max-h-[85vh]">
              ...
              <div className="grid grid-cols-2 gap-4 overflow-y-auto">
                ...
                <Link to="/settings" onClick={() => setIsMobileMenuOpen(false)} className="flex flex-col items-center gap-3 p-5 rounded-3xl bg-indigo-50/50 dark:bg-indigo-900/20">
                ...
                >
                  <div className="text-emerald-600 dark:text-emerald-400"><Share2 size={22} /></div>
                  <span className="font-bold text-[10px] text-emerald-700 dark:text-emerald-300 uppercase tracking-widest">{language === 'pt' ? 'Compartilhar Sistema' : 'Share System'}</span>
                </button>
              </div>
            </div>
          </div>
            ...
            {academy ? (
              <>
                ...
                <Route path="/staff" element={
                  <ProtectedRoute roles={['superuser', 'admin']} user={user}>
                    <StaffView academy={academy} user={user} />
                  </ProtectedRoute>
                } />
                <Route path="/inventory" element={
                  <ProtectedRoute roles={['superuser', 'admin', 'instructor', 'staff', 'student']} user={user}>
                    <InventoryView academy={academy} user={user} />
                  </ProtectedRoute>
                } />
                ...
                <Route path="/instructor-profile" element={
                  <ProtectedRoute roles={['instructor', 'staff']} user={user}>
                    <InstructorProfileView academy={academy} user={user} />
                  </ProtectedRoute>
                } />
    ...
    <aside className={`hidden md:flex flex-col fixed left-0 top-0 bottom-0 bg-slate-900 text-white transition-all duration-300 z-50 shadow-2xl ${isCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      <div className="mt-auto space-y-1 p-4 border-t border-slate-800 bg-slate-900/50">
        ...
        <button 
          ...
          className={`flex items-center text-slate-400 hover:text-emerald-400 h-10 rounded-xl hover:bg-slate-800/50 ${isCollapsed ? 'justify-center w-10 mx-auto' : 'px-4 gap-3'}`}
        >
          <Share2 size={18} className="text-emerald-500" />
          {!isCollapsed && <span className="font-bold text-xs">{language === 'pt' ? 'Compartilhar Sistema' : 'Share System'}</span>}
        </button>
      </div>
    </aside>
  );

...
- **E-mail duplicado entre alunos/instrutores/colaboradores** — nada impedia duas fichas na mesma academia com o mesmo e-mail (foi a causa raiz do bug anterior). Corrigido em duas camadas:
  1. Validação na API em todos os pontos de criação/edição (`POST/PUT /api/students`, `POST/PUT /api/instructors`, `POST /api/auth/register/student`, `POST /api/auth/register/instructor`, `POST /api/auth/register/staff`) — rejeita com 409 se o e-mail já pertence a outra ficha da mesma academia.
  2. Constraint `UNIQUE(academy_id, email)` no banco (`students`, `instructors`, `staff`) — trava de segurança mesmo que algum código futuro esqueça de validar. `NULL` continua permitido em qualquer quantidade (alunos sem e-mail não são afetados).
  - Auditoria em QAS e PRD: **PRD já estava limpo** (zero duplicados). **QAS tinha 3 grupos antigos** de dados de teste/seed (`andre@teste.com.br`, `contatotnadigital@gmail.com`, `aluno@samurai.com`) — limpos (mantido o e-mail no primeiro registro de cada grupo, removido dos demais) antes de aplicar a constraint, para não travar a migração.
- **Dados misturados entre aluno e dependente** — vários pontos do frontend resolviam "quem é o usuário logado" comparando **e-mail** (`students.find(s => s.email === user.email)`), incluindo alguns que eu mesmo tinha deixado assim na Fase 3. Como a ficha da Maria e a do Theo tinham o mesmo e-mail de contato cadastrado, o sistema pegava a ficha errada mesmo com "Theo" selecionado (a saudação usava o nome da conta direto, então parecia certa, mas o card/faixa/mensalidade vinham da outra pessoa). Troquei a resolução para comparar por **userId** (não ambíguo) em todos os pontos encontrados: Dashboard (saudação, ID digital, mensalidade, "Treinos no Mês"), Configurações → Meus Dados, e a ficha de Instrutor/Staff. O e-mail continua como fallback só para contas ainda não vinculadas a um `user_id`.

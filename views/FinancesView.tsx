...
const FinancesView: React.FC<{ academy: Academy; user: User }> = ({ academy }) => {
  ...
  const categories = useMemo(() => ({
    income: [t.tuition, t.beltExam, t.seminar, t.equipmentSale, t.others],
    expense: [t.rent, t.utilities, t.cleaning, t.marketing, t.teacherSalary, t.maintenance, t.others]
  }), [t]);

  const paymentMethodsList = useMemo(() => [t.cash, t.pix, t.creditCard, t.debitCard, t.transfer], [t]);
    ...
    try {
      ...
      const canvas = await html2canvas(financeRef.current, {
        ...
        onclone: (clonedDoc) => {
          ...
          rootElement.innerHTML = rootElement.innerHTML.replace(/okl(ch|ab)\s*\([^)]+\)/g, '#333333');

          // 2. Clear common variables that might still exist in the CSSOM
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-32 animate-in fade-in duration-500">
      <div ref={financeRef} className="bg-white dark:bg-slate-800 p-4 md:p-8 rounded-[48px] print:p-0">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 no-print mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">{t.cashFlow}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-slate-900 dark:bg-slate-800 text-white p-6 rounded-[40px] shadow-xl relative overflow-hidden group border border-slate-700">
           <div className="relative z-10">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60 mb-1">{t.monthProjection}</p>
              <h3 className="text-2xl font-black">
                {new Intl.NumberFormat(language === 'pt' ? 'pt-BR' : language === 'es' ? 'es-ES' : 'en-US', { style: 'currency', currency: language === 'pt' ? 'BRL' : 'USD' }).format(students.filter(s => s.status === 'Active').length * 150)}
              </h3>
              <p className="text-[8px] font-bold text-indigo-400 uppercase tracking-widest mt-1 italic">{t.expectancyReality}</p>
           </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6 no-print">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm transition-colors">
            <h3 className="font-bold text-slate-800 dark:text-white mb-4">{t.filterHistory}</h3>
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <FilterButton label={t.all} active={typeFilter === 'All'} onClick={() => setTypeFilter('All')} />
                <FilterButton label={t.income} active={typeFilter === 'income'} onClick={() => setTypeFilter('income')} />
                <FilterButton label={t.expenses} active={typeFilter === 'expense'} onClick={() => setTypeFilter('expense')} />
              </div>
            </div>
          </div>
        </div>
          ...
          {filteredTransactions.map(t => (
            <div key={t.id} className="p-6 flex flex-col gap-3 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{new Date(t.date).toLocaleDateString(language === 'pt' ? 'pt-BR' : language === 'es' ? 'es-ES' : 'en-US')}</span>
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
              <tr>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.dateLabel}</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.description}</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.category}</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.amountLabel}</th>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {filteredTransactions.map(t => (
                <tr key={t.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="px-8 py-5 text-sm font-bold text-slate-500 whitespace-nowrap">{new Date(t.date).toLocaleDateString(language === 'pt' ? 'pt-BR' : language === 'es' ? 'es-ES' : 'en-US')}</td>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[150] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[40px] p-8 animate-in zoom-in duration-300 shadow-2xl text-center">
            <header className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-black text-slate-800 dark:text-white uppercase italic tracking-tight">{t.digitalBilling}</h2>

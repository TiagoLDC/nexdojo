...
const CalendarView: React.FC<{ academy: Academy; user: User }> = ({ academy, user }) => {
  ...
  const monthName = currentDate.toLocaleString(monthLocale, { month: 'long' });
  ...
  const weekendDays = language === 'pt' ? ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'] : language === 'en' ? ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] : ['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb'];
              ...
              {user.role === 'student' ? (
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-5 rounded-3xl shadow-xl shadow-red-600/20 transition-all flex items-center justify-center gap-2"
                  >
                    <Trash2 size={20} />

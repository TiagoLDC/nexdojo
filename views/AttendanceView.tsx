...
const AttendanceView: React.FC<{ academy: Academy; user: User }> = ({ academy, user }) => {
  ...
  const handleCheckIn = (student: Student) => {
    ...
    // Check for duplicate in the same training slot (same day)
    const todayStr = new Date().toISOString().split('T')[0];
    const isAlreadyMarkedToday = StorageService.getAttendance(academy.id).some(r => {
      return r.studentId === studentId && r.date.split('T')[0] === todayStr && r.classId !== currentClass?.id;
    });

    if (isAlreadyMarkedToday && !checkedIds.has(studentId)) {
      if (!confirm(`O aluno ${student.name} já registrou presença em outro treino hoje. Deseja registrar novamente?`)) {
        return;
      }
    }
    ...
    if (currentClass) {
      ...
      // Update local currentClass state to keep it in sync
      const updatedClass = { ...currentClass, attendanceIds };
      setCurrentClass(updatedClass);
      
      // Update storage and global classes list
      const refreshedClasses = StorageService.getClasses(academy.id);
      const updatedClasses: ClassSession[] = refreshedClasses.map(c => 
        c.id === currentClass.id ? updatedClass : c
      );
      StorageService.saveClasses(updatedClasses, academy.id);
      setAllClasses(updatedClasses);
    }

    // Feedback for check-in
    if (!isAlreadyChecked) {
  ...
  const handleSelectStudent = (student: Student) => {
    if (checkedIds.has(student.id)) {
      // If already checked, just toggle off (remove from mat)
      handleCheckIn(student);
    } else {
      // If not yet checked, ask for confirmation as requested
  ...
  const startClass = (name: string, templateId?: string, duration = 60) => {
    ...
    StorageService.saveClasses(updated, academy.id);
  ...
  const handleDiscardClass = async () => {
    ...
    // Just close the session, the handleCheckIn already saves the current attendanceIds to storage
  ...
  const handleDeleteCurrentClass = async () => {
    ...
    if (confirm(t.confirmDeleteClass || "Tem certeza que deseja apagar esta aula e todos os registros nela?")) {
      ...
      // Also remove attendance records if it was finalized
      if (currentClass.status === 'Finalized') {
        const attendance = StorageService.getAttendance(academy.id);
        const filteredAttendance = attendance.filter(r => r.classId !== currentClass.id);
        StorageService.saveAttendance(filteredAttendance, academy.id);
      }
  ...
  const handleFinishClass = async () => {
    ...
    try {
      ...
      const studentsToIncrement = currentAttendanceIds.filter(id => !editingClassInitialIds.has(id));
      ...
      if (isUpdate) {
        ...
        const newRecords: AttendanceRecord[] = currentAttendanceIds.map((studentId: string) => ({
        ...
        StorageService.saveAttendance([...otherRecords, ...newRecords], academy.id);
      } else {
        const newRecords: AttendanceRecord[] = currentAttendanceIds.map((studentId: string) => ({
          id: Math.random().toString(36).substr(2, 9),
          academyId: academy.id,
        ...
        StorageService.saveAttendance([...academyRecords, ...newRecords], academy.id);
      }

      const academyClasses: ClassSession[] = StorageService.getClasses(academy.id).map(c => 
        c.id === currentClass.id ? { ...c, attendanceIds: currentAttendanceIds, status: 'Finalized' } : c
      );
      StorageService.saveClasses(academyClasses, academy.id);
      ...
      const currentTemplateId = currentClass.templateId;
      const currentTemplate = currentTemplateId ? academyTemplates.find(t => t.id === currentTemplateId) : null;
  ...
  const handleQuickFinalize = (session: ClassSession) => {
    ...
    try {
      ...
      // Filter out any existing records for this class to avoid duplicates
      const otherRecords = academyRecords.filter(r => r.classId !== session.id);
      
      // Create new attendance records
      const newRecords: AttendanceRecord[] = attendanceIds.map((studentId: string) => ({
        id: Math.random().toString(36).substr(2, 9),
        academyId: academy.id,
        studentId,
        classId: session.id,
      ...
      // Update session status in storage and local state
      const academyClasses = StorageService.getClasses(academy.id).map(c => 
        c.id === session.id ? { ...c, status: 'Finalized' as const } : c
      );
      StorageService.saveClasses(academyClasses, academy.id);
      setAllClasses(academyClasses);
      
      // Update student stats and handle evasion alerts
      const academyStudents = StorageService.getStudents(academy.id);
      const academyTemplates = StorageService.getTemplates(academy.id);
      const sessionTemplate = session.templateId ? academyTemplates.find(tem => tem.id === session.templateId) : null;
      const defaultLimit = academy?.absenceLimit || 3;
      const studentsWithAlert: string[] = [];

      const updatedStudents = academyStudents.map(s => {
        if (attendanceIds.includes(s.id)) {
          return {
            ...s,
            totalClasses: (s.totalClasses || 0) + 1,
            totalHours: (s.totalHours || 0) + ((session.durationMinutes || 60) / 60),
            lastAttendance: date,
            absentCount: 0
          };
        } else if (s.status === 'Active') {
          // Count absence for those who didn't attend
          const newAbsentCount = (s.absentCount || 0) + 1;
          const effectiveLimit = s.absenceLimit || sessionTemplate?.absenceLimit || defaultLimit;
      ...
      StorageService.saveStudents(updatedStudents, academy.id);
      setStudents(updatedStudents); // Keep local state in sync
      
      if (studentsWithAlert.length > 0) {
        alert(`${t.evasionAlert || 'Alerta de Evasão'}:\n\nOs seguintes alunos atingiram seus limites de faltas:\n\n- ${studentsWithAlert.join('\n- ')}\n\n${t.considerContacting || 'Considere entrar em contato.'}`);
      }
      
      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 3000);
      
      // If we are currently editing THIS class, we should also update currentClass state
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-40 animate-in fade-in duration-300">
      <header className="sticky top-0 bg-slate-50 dark:bg-slate-950 pt-2 z-10 space-y-4 pb-4 px-2">
        ...
        {/* Mobile-only action button - REMOVED redundant buttons from header */}

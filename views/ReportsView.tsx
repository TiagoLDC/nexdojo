    ...
    try {
      ...
      const canvas = await html2canvas(reportRef.current, {
        ...
        onclone: (clonedDoc) => {
          ...
          // 2. Inject explicit fallback variables for common Tailwind colors
          const style = clonedDoc.createElement('style');
          style.innerHTML = `
            * {
              /* Forcing fallback colors for common Tailwind classes */
        ...
        <div className="lg:col-span-3 bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <div className="flex flex-wrap gap-2">
            <button 
              ...
              className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center gap-2 whitespace-nowrap"
            >
              <Clipboard size={12} /> Copiar Todos
            </button>
            <div className="bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center">
              {graduationPipeline.length} Atletas
            </div>
          </div>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {graduationPipeline.map(student => (
                  <tr key={`${student.id}-${student.academyId}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">

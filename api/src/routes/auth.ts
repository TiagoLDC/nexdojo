...
router.post('/register/student', async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    await pool.execute(
      `INSERT INTO students (id, academy_id, name, email, belt, stripes, birth_date, gender, phone, cpf, rg, weight, height, blood_type, emergency_contact, emergency_phone, cep, address, address_number, guardian_name, guardian_phone, guardian_relation, guardian_cpf, medical_notes, photo, plan_id, status, join_date, total_classes, total_hours, absent_count)
      ...
      [studentId, academyId, name, String(email).toLowerCase().trim(), belt, stripes || 0, birthDate,
       ...
       guardianCpf || null, medicalNotes || null, photo || null, planId]
    );
    await pool.execute(

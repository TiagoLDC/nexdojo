...
- **Descoberta importante feita ao decidir onde plugar a tela**: o projeto tem **dois `App.tsx` distintos**. O `index.tsx` (raiz) monta o `App.tsx` da **raiz** (`HashRouter` + `views/*.tsx` + `types.ts` legado) — esse é o app que roda de verdade. Já existe também um `src/App.tsx` (React Router com `<Routes>` apontando para `src/pages/*Page.tsx`, que por sua vez envolvem `src/features/*/views/*`) — **esse não é importado por nada, é código morto/uma migração não finalizada**. Prova: `UsersView`/`SystemConfigView` (as versões novas em `src/features/*`) não aparecem em nenhum import do `App.tsx` da raiz. Implementação desta fase seguiu o app que **realmente roda** (raiz). Achado registrado aqui só como observação — não foi mexido/limpo, é decisão de outra frente.
  > ⚠️ **CORREÇÃO (24/08/2026)**: esta conclusão estava **invertida**. `index.html` carrega `/src/main.tsx`, não `/index.tsx` da raiz — quem roda de verdade é `src/App.tsx` (BrowserRouter + `src/pages/*Page.tsx`), e o `App.tsx`/`index.tsx` da raiz é que são código morto. Ver seção "Descoberta de app duplicado (24/08/2026)" mais abaixo para os detalhes completos e o impacto real. Este erro de diagnóstico, carregado por 2 semanas na documentação, fez a Fase 5 plugar a rota `/sports` só no app morto — corrigido na mesma sessão de 24/08.
...
- **Rota e navegação**: `/sports` adicionada em `App.tsx` (raiz) **fora** do bloco `{academy ? (...) : ...}`, já que é uma tela global (independe de academia selecionada), protegida por `ProtectedRoute roles={['superuser']}`. Link de acesso colocado dentro do painel "Master Control" da Sidebar (o mesmo bloco que já tem o seletor de academia do superuser) — visível com ou sem academia selecionada.
  > ⚠️ **CORREÇÃO (24/08/2026)**: isso foi plugado no `App.tsx` da raiz, que é código morto (ver correção na Fase 5 acima e a seção "Descoberta de app duplicado" mais abaixo) — a tela nunca existiu de verdade para nenhum usuário entre 11/08 e 24/08/2026. Refeito corretamente em `src/App.tsx` + `src/pages/SportsPage.tsx` + `src/components/layout/navConfig.ts`.
...
### Correção pós-plano (13/08/2026) — "tempo" não zerava na promoção pelo cadastro

**Pergunta do usuário que motivou a checagem**: se o aluno for promovido tanto pelo cadastro (edição direta de faixa/grau) quanto pelo módulo de promoção (Central de Graduação), aulas e tempo devem zerar para contar de novo até a próxima promoção.

**Verificação feita**: os dois caminhos de promoção em `api/src/routes/students.ts` foram revisados.
- **`POST /:id/graduate`** (módulo de promoção, botão "Promover") — já estava correto: grava `last_graduation_date = date` (a data enviada, hoje por padrão no frontend) e zera `classes_since_graduation`/`hours_since_graduation` na mesma instrução.
- **`PUT /:id`** (cadastro — editar faixa/grau direto no formulário do aluno) — **tinha um bug real**: `classes_since_graduation`/`hours_since_graduation` eram zerados corretamente, mas `last_graduation_date` **não avançava para hoje**. Causa: o formulário de edição reenvia o cadastro inteiro (`{ ...editingStudent }`), então `last_graduation_date` quase sempre chegava preenchido no payload com o valor **antigo** (não por escolha deliberada do admin) — e o backend só usava `getTodayBrasilia()` como padrão quando o campo vinha **totalmente ausente**, o que quase nunca acontecia. Resultado: um aluno promovido pelo cadastro tinha os contadores de aulas zerados, mas o "tempo desde a última graduação" continuava contando da data antiga — quebrava especificamente as academias que usam **meses** como critério (o aluno podia aparecer imediatamente elegível de novo, ou com progresso incorreto).

**Correção**: `gradDate` agora só usa a data recebida no payload quando ela **realmente difere** da que já estava salva (`existing[0].last_graduation_date`) — sinal de que o admin editou a data de propósito (ex: corrigindo um registro histórico). Fora esse caso, assume hoje, igual ao módulo de promoção. `last_graduation_date` passou a ser atualizado explicitamente na mesma instrução que zera os contadores, em vez de depender do fluxo genérico de campos do formulário.
...
### Verificação empírica (13/08/2026) — zeramento de aulas/data na promoção, nos dois caminhos

**Pedido do usuário**: confirmar, com dados reais (não só revisão de código), que tanto a promoção pelo cadastro quanto pela Central de Graduação zeram aulas e a data da última graduação.

**Verificação feita**: consultado o QAS diretamente (`graduation_history` cruzado com o estado atual de `students`, pegando a promoção mais recente de cada aluno).
- **Módulo de promoção** (`notes` vazio — não veio do cadastro): 6 promoções reais feitas hoje (13/08) por um usuário de teste, todas com `classes_since_graduation = 0`, `hours_since_graduation = 0` e `last_graduation_date` = hoje. **Confirmado funcionando corretamente com uso real, pós-deploy da correção.**
- **Cadastro** (`notes = 'Atualização manual'`): os registros mais recentes por esse caminho são de antes da correção (até 12/08). Achado interessante que **confirma exatamente o diagnóstico do bug**: promoções onde o aluno nunca tinha `last_graduation_date` preenchido (primeira graduação) ficaram com a data **nula** para sempre (o código antigo nunca escrevia a data nesse caso) — evidência real do bug relatado. Promoções onde o admin por acaso preencheu a data no formulário saíram corretas por coincidência. Isso bate 100% com a causa raiz já identificada e corrigida no commit `#208`.
- **Não foi possível** encontrar, ainda, uma promoção real pelo cadastro feita **depois** do deploy da correção (ninguém usou esse caminho específico desde então) — a lógica foi confirmada por revisão de código (é estruturalmente a mesma lógica já comprovada funcionando no módulo de promoção, só com a guarda adicional de "a data foi alterada de propósito?"), mas ainda não com um caso real pós-fix. Recomendado ao usuário testar uma edição de faixa/grau pelo cadastro para confirmação final com dado real.

### Substituição de card no dashboard do aluno (13/08/2026)

O card "Eventos & Recessos" (atalho para `/calendar`, redundante com o menu principal) foi substituído por um card de estatística **"Aulas Nessa Faixa"**, mostrando `classesSinceGraduation` — dá ao aluno visibilidade direta de quantas aulas já fez desde a última graduação, no mesmo grid dos outros cards (Total de Aulas, Treinos no Mês, Total de Horas, Faltas). `views/DashboardView.tsx`: `motion.button` de navegação trocado por `StatCard` (mesmo componente dos outros 4 cards do grid), ícone `Activity` (mesmo já usado para esse campo em `StudentsView.tsx`, por consistência). `tsc --noEmit` idêntico à baseline, zero erros novos.
...
## Descoberta de app duplicado (24/08/2026) — corrige um erro de diagnóstico de 2 semanas

**Como foi descoberto**: depois do deploy da segmentação por idade/grau (seção acima), o usuário reportou não achar a tela "Esportes" em lugar nenhum, logado como superuser confirmado no banco (QAS e PRD). A investigação revelou que a Fase 5 (11/08/2026) tinha **diagnosticado errado** qual dos dois `App.tsx` do projeto é o que realmente roda no navegador — e vinha sendo levado adiante como verdade desde então.

**O erro**: a Fase 5 concluiu que `index.tsx`/`App.tsx` da raiz (`HashRouter`, com Sidebar/menu mobile embutidos ali dentro) era o app real, e que `src/App.tsx` (`BrowserRouter` + `src/pages/*Page.tsx`) era código morto. É o **contrário**: `index.html` carrega `<script src="/src/main.tsx">` — não `/index.tsx`. `src/main.tsx` monta `src/App.tsx`. O `App.tsx`/`index.tsx` da raiz nunca são executados.

**Por que isso não quebrou tudo silenciosamente**: por sorte de arquitetura, não por intenção — a maioria dos `src/pages/*Page.tsx` (`SettingsPage`, `DashboardPage`, `StudentsPage`, `ReportsPage`, `StudentProfilePage`, `InstructorProfilePage`) são wrappers finos que **importam e renderizam os mesmos `views/*.tsx`** que o `App.tsx` da raiz também usa (ex: `SettingsPage.tsx` → `import SettingsView from '../../views/SettingsView'`), só trocando a origem dos dados (Zustand em vez de props vindas do `App.tsx` raiz). Ou seja: **editar `views/*.tsx` sempre teve efeito real na aplicação**, mesmo com o diagnóstico de qual `App.tsx` roda estando invertido. É por isso que toda a segmentação por idade/grau (`views/SettingsView.tsx`, `services/graduation.ts`, `views/DashboardView.tsx`, etc.) já estava funcionando de verdade no QAS antes desta correção.

**O que realmente ficou quebrado**: só o que dependia especificamente do `App.tsx`/Sidebar/menu mobile da **raiz** — ou seja, só a tela **Esportes** (`views/SportsView.tsx`), que é acessada por rota/link de navegação, não por um `*Page.tsx` wrapper que já existisse. A Fase 5 tinha adicionado a rota `/sports` e o link "Esportes" só dentro do `App.tsx` da raiz (código morto) — por isso a tela nunca existiu de verdade para nenhum usuário, desde 11/08/2026.

**Correção aplicada (24/08/2026)**:
- `src/pages/SportsPage.tsx` (novo) — wrapper fino no mesmo padrão de `SettingsPage.tsx`, importando `views/SportsView.tsx`.
- `src/App.tsx` — rota `/sports` adicionada dentro do `<RoleGuard roles={['superuser']} />`, mesmo grupo de `/system-config`.
- `src/components/layout/navConfig.ts` — item `{ to: '/sports', labelKey: 'sports', icon: 'Trophy', roles: ['superuser'] }` em `MANAGEMENT_NAV`. Como tanto a Sidebar de desktop (`src/components/layout/Sidebar.tsx`) quanto o menu mobile (`src/components/layout/MobileMenu.tsx`) leem dessa mesma lista, um único item de config cobre os dois lugares.
- `src/components/layout/NavIcons.tsx` — ícone `Trophy` adicionado ao mapa.
- `services/translations.ts` — chave `sports` adicionada nos 3 idiomas (pt: "Esportes", en: "Sports", es: "Deportes").
- `index.tsx` e `App.tsx` da raiz — marcados com comentário de aviso no topo do arquivo (código morto, não editar esperando efeito na UI real), a pedido do usuário, para não repetir esse erro de diagnóstico numa sessão futura. Não foram removidos ainda — a exclusão definitiva fica para uma limpeza futura (ver observação abaixo).

**Pendência para limpeza futura**: os arquivos `index.tsx` e `App.tsx` da raiz continuam no repositório, mortos, só comentados. Removê-los de vez exige antes confirmar que nenhum script de build/deploy os referencia diretamente (checado `package.json` e `vite.config.ts` nesta sessão — nenhuma referência direta, o Vite só constrói o que é alcançável a partir de `index.html`), e depois apagar os dois arquivos + o `<Sidebar>`/menu mobile embutidos dentro do `App.tsx` da raiz (que não têm equivalente a preservar, já foram substituídos por `src/components/layout/Sidebar.tsx`/`MobileMenu.tsx`).

**Verificação**: `npx tsc --noEmit` — mesmos 68 erros pré-existentes, zero novos nos arquivos tocados (`src/App.tsx`, `src/pages/SportsPage.tsx`, `navConfig.ts`, `NavIcons.tsx`).
